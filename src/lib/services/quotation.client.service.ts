...
class QuotationClientService {
  async fetchQuotations(params: FetchQuotationsParams, retryCount = 0): Promise<QuotationsResponse> {
    try {
        ...
        switch (response.status) {
          ...
          case 429:
            if (retryCount < MAX_RETRIES) {
              ...
              return this.fetchQuotations(params, retryCount + 1);
            }
  ...
  // async createQuotation(data: CreateQuotationData): Promise<QuotationResponse>;
  // async updateQuotation(id: string, data: UpdateQuotationData): Promise<QuotationResponse>;
  // async deleteQuotation(id: string): Promise<void>;

  private handleApiError(response: Response, data: unknown): never {
    console.error("API error:", response.status, data);
    throw new ApiError(`API error occurred: ${response.statusText}`, response.status);
  }
}
